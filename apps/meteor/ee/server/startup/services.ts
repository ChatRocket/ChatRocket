...
api.registerService(new EnterpriseSettings(), ['license']);
api.registerService(new LDAPEEService(), ['license']);
...
if (!isRunningMs()) {
	api.registerService(new InstanceService(), ['license']);
}
...
export const startFederationService = async (): Promise<void> => {
	...
	if (!License.hasValidLicense()) {
		...
		api.registerService(federationService, ['license']);
	}
	...
	void License.onLicense('federation', async () => {
		...
		api.registerService(federationServiceEE, ['license']);
	});
};
